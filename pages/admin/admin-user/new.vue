	...
	export default {
		...
		methods: {
			async createAdminUser(registrationInfo) {
				try {
					const registerInstructor = await this.$axios.post('/admin/add', registrationInfo);

					if (registerInstructor.status === 200 || registerInstructor.status === 204) {
					...
					this.$store.dispatch('snackbar/setSnackbar', { text: `Cannot Create Instructor` })
				}
			},
		},
	};
